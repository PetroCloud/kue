...
Queue.prototype.process = function (type, n, fn) {
    ...
    if ('function' == typeof n){
        fn = n, workersConfig = {n: 1, shared: false};
    } else if ('number' == typeof n) {
        workersConfig.n = n;
        workersConfig.shared = false;
    } else if ('object' == typeof n) {
        workersConfig = n;
    }

    while (workersConfig.n--) {
        var worker = new Worker(this, type, workersConfig).start(fn);
...
/**
 * Return job ids with the given `state`, and specified equipment, and callback `fn(err, ids)`.
 *
 * @param {String} state
...
Queue.prototype.state = function (state, equipment, fn) {
    this.client.zrange(this.client.getKey('jobs:' + equipment + ":" + state), 0, -1, fn);
    return this;
};
...
//----------------
/**
 * Completed jobs by equipment.
 */

Queue.prototype.complete = function (equipment, fn) {
    return this.state('complete', equipment, fn);
};

/**
 * Failed jobs by equipment.
 */

Queue.prototype.failed = function (equipment, fn) {
    return this.state('failed', equipment, fn);
};

/**
 * Inactive jobs (queued) by equipment.
 */

Queue.prototype.inactive = function (equipment, fn) {
    return this.state('inactive', equipment, fn);
};

/**
 * Active jobs (mid-process) by equipment.
 */

Queue.prototype.active = function (equipment, fn) {
    return this.state('active', equipment, fn);
};

/**
 * Delayed jobs by equipment.
 */

Queue.prototype.delayed = function (equipment, fn) {
    return this.state('delayed', equipment, fn);
};
//---------------------------------
