...
function Worker(queue, type, workerConfig) {
    this.queue = queue;
    this.type = type;
    this.shared = workerConfig.shared;
    this.numWorkers = workerConfig.n;
...
Worker.prototype.getJob = function(fn) {
    ...
    client.blpop(client.getKey(self.type + ':jobs'), 0, function(err) {
        ...
        self.job = true;
        self.zpop(self.client.getKey('jobs:' + self.type + ':inactive'), function(err, idInactive) {
            if (err || !idInactive) {
                self.job = null;
                return fn(err /*|| "No job to pop!"*/ );
            }
            if (self.shared) {
                self.client.zcount(self.client.prefix + ':jobs:' + self.type + ':active', '-inf', 'inf', function(err, numActive) {
                    if (err) {
                        return fn(err /*|| "No job to pop!"*/ );
                    }
                    if (!numActive) {
                        Job.get(idInactive, fn);
                    } else if (self.numWorkers > numActive) {
                        Job.get(idInactive, fn);
                    } else {
                        Job.get(idInactive, function(err, job) {
                            job.inactive();
                            setTimeout(function() {
                                return fn(null /*|| "No job to pop!"*/ );
                            }, 5000);
                        });
                    }
                });
